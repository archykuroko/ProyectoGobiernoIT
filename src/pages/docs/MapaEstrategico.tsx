              ...
              return (
                <div key={layer.c} style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 24, padding: '32px 0', borderTop: '1px solid var(--line)', alignItems: 'start' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 10 }}>
                    {layer.strs.map((s, i) => (
                      <div key={i} style={{ padding: '10px 12px', background: 'var(--bg-2)', border: '1px solid var(--line)', borderLeft: `2px solid ${lc}`, borderRadius: 6, fontSize: 12, lineHeight: 1.42, color: '#d4d9e0' }}>
                        {s}
                      </div>
                    ))}
                  </div>
